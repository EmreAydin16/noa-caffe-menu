/**
 * SADECE gorselleri duzeltir. Canli Supabase verisini okur, fiyatlara dokunmaz.
...
async function readLiveMenu() {
    const res = await fetch(`${SUPABASE_URL}/storage/v1/object/${BUCKET}/${FILE}`, {
        headers: headers()
    });
    if (!res.ok) throw new Error(`Supabase okunamadi: ${res.status} ${await res.text()}`);
    return res.json();
}

async function writeLiveMenu(menu) {
    const res = await fetch(`${SUPABASE_URL}/storage/v1/object/${BUCKET}/${FILE}`, {
        method: 'POST',
        headers: headers({
...
async function main() {
    const menu = await readLiveMenu();
    let updated = 0;

    for (const cat of menu.categories) {
        for (const item of cat.items) {
            if (FIX[item.id] !== undefined && item.image !== FIX[item.id]) {
    ...
    await writeLiveMenu(menu);
    console.log(`${updated} gorsel guncellendi (fiyatlar korundu)`);
}
